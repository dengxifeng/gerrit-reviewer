...
- The webhook transform is installed by copying `hooks/transforms/gerrit-review.js` into `~/.openclaw/hooks/transforms/`; the `init` command also configures `openclaw.json` hooks section (`enabled`, `token`, `path`, `allowRequestSessionKey`, `mappings`) by editing the file directly, generating a random token with `secrets.token_hex(24)`, and enables ACP (`acp.enabled`) and the `acpx` plugin (`plugins.entries.acpx.enabled`)
